...
class Mega6SensorNode(Node):
    def __init__(self):
        super().__init__('m6sensor_node')
        
        # Publishers (matching GUI expectations)
        self.response_publisher = self.create_publisher(String, 'mega6/sensor_response', 10)
        self.state_publisher = self.create_publisher(Bool, 'mega6/sensor_state', 10)
        
        # Subscriber for sensor commands
        self.command_subscription = self.create_subscription(
            String,
            'mega6/sensor_command',
            self.command_callback,
            10
        )
        
        # Arduino connection settings
        self.arduino_ip = '192.168.100.106'
        ...
        self.get_logger().info('Mega6 Sensor node started. Polling at 50Hz.')
