...
class Mega1GUI(Node):
    def __init__(self):
        super().__init__('mega1_gui_node')
        self.relay_publisher = self.create_publisher(String, 'mega1/relay_command', 10)
        self.relay_status_subscriber = self.create_subscription(String, 'mega1/relay_status', self.update_relay_status, 10)
        self.sensor_status_subscriber = self.create_subscription(String, 'mega1/sensor_response', self.update_sensor_status, 10)

        self.root = tk.Tk()
        self.root.title("Mega1 Control Panel")
