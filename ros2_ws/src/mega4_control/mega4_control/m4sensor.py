...
class Mega4SensorNode(Node):
    def __init__(self):
        super().__init__('m4sensor_node')
        
        # Publishers (matching GUI expectations)
        self.response_publisher = self.create_publisher(String, 'mega4/sensor_response', 10)
        self.state_publisher = self.create_publisher(Bool, 'mega4/sensor_state', 10)
        
        # Subscriber for sensor commands
        self.command_subscription = self.create_subscription(
            String,
            'mega4/sensor_command',
            self.command_callback,
            10
        )
        
        # Arduino connection settings
        self.arduino_ip = '192.168.100.104'
        ...
        # Timer for periodic sensor readings (reduced frequency)
        self.sensor_timer = self.create_timer(0.02, self.read_sensor)  # 50Hz
        
        self.get_logger().info('Mega4 Sensor node started. Polling at 50Hz.')
